...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancel) return;
        setWallets(list);
        const primaryW = list.find(w => w.isPrimary);
        if (primaryW) setWalletId(primaryW.id);
    ...
    (async () => {
      ...
      try {
        const [
          ...
          apiCall.get("Dashboard/StatusSplit",  { params: { ...params, type: "income", groupBy: granularity } }),
          apiCall.get("Dashboard/StatusSplit",  { params: { ...params, type: "expense", groupBy: granularity } }),
          apiCall.get("Dashboard/Categories",   { params: { ...params, type: "income" } }),
          apiCall.get("Dashboard/Categories",   { params: { ...params, type: "expense" } }),
          apiCall.get("Dashboard/WalletBalances",{ params }),
        ]);
  ...
  return (
    <div className="space-y-6 min-h-screen">
      ...
      >
        <div
          className="flex items-center gap-2 border rounded-xl px-3 py-2 min-w-[16rem] sm:min-w-[20rem]"
        ...
        >
          ...
          <select
            ...
            title={
              ...
              (wallets.find(w => w.id === walletId)?.isPrimary ? ` (${t("dashboard.filters.wallet_primary")})` : "")
            }
          >
            {wallets.map(w => (
              <option
        ...
        {/* From */}
        <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{ borderColor: border, background: bg }}>
          <Calendar className="w-4 h-4" />
          <input type="date" value={toISO(from)} onChange={e => setFrom(new Date(e.target.value))} className="bg-transparent outline-none" />
        </div>

        {/* To */}
        <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{ borderColor: border, background: bg }}>
          <Calendar className="w-4 h-4" />
          <input type="date" value={toISO(to)} onChange={e => setTo(new Date(e.target.value))} className="bg-transparent outline-none" />
        </div>
