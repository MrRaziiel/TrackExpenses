...
function ListExpenses() {
    ...
    const fetchExpenses = async () => {
      try {
        const res = await apiCall.get(
          `Expenses/ListExpenses?userEmail=${auth?.email}`
        );
        const enriched =
  ...
  const handleDeleteExpense = async (expenseId) => {
    if (!window.confirm("Deseja apagar esta despesa e todas as instâncias?"))
      return;
    ...
    try {
      ...
      setExpenses((prev) =>
        prev.map((exp) => ({
          ...
          instances: {
            $values:
              exp.instances?.$values?.filter((i) => i.id !== instanceId) || [],
          },
        }))
      );
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <Title
          text={
            t("common.expenses") ||
            "Desbloqueia todas as funcionalidades sem limites."
          }
        />
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-xs font-medium text-left uppercase tracking-wider">
                {t("common.name")}
              </th>
              <th className="px-6 py-3 text-xs font-medium text-left uppercase tracking-wider">
                {t("common.category")}
              ...
              <th className="px-6 py-3 text-xs font-medium text-left uppercase tracking-wider">
                {t("common.value")}/{t("common.notpayed")}
              </th>
              <th className="px-6 py-3 text-xs font-medium text-left uppercase tracking-wider">
                {t("common.actions")}
              </th>
                ...
                <React.Fragment key={exp.id}>
                  <tr className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-red-600">
                      -{calculateRemaining(exp)}€ /{" "}
                      {Number(exp.value || 0).toFixed(2)}€
                    </td>
